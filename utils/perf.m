...
or = [1 0.5 0];
p = [0.5 0 64/255];
grey1 = [0.1490 0.1490 0.1490];
grey2 = [ 0.5020 0.5020 0.5020];
grey3 = [0.6510 0.6510 0.6510];

colors  = {grey1 p or 'y' 'm' 'g' 'c'};%  p 'm' 'y'}; 
lines   = {'-' '--' '-.' '-' '-.' '-' '--' '-'};
markers = {'x' '*' 'p' 'd' 'x' '+' 'v' 'p' '^'}; % 
...
for s = 1: ns
 ...
 %option = [lines(s),  markers(s) ];
 plot(xs,ys, 'LineStyle',lines{s},'Color',colors{s},'Marker', markers{s},'MarkerSize',3);
 hold on;
end
...
xlabel('$\theta$','FontSize',15,'Interpreter','Latex'); 
ylabel('$\rho(\theta)$','FontSize',15,'Interpreter','Latex');
